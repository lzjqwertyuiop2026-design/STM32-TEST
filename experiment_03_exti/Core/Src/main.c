...
/* USER CODE BEGIN PV */
static uint32_t wkupLastTick = 0u;
static uint32_t key0LastTick = 0u;
static uint32_t key1LastTick = 0u;
...
static uint8_t IsDebounced(uint32_t *lastTick);
...
static uint8_t IsDebounced(uint32_t *lastTick)
{
  ...
  if ((*lastTick == 0u) || ((now - *lastTick) >= DEBOUNCE_MS))
  {
    *lastTick = now;
    return 1u;
  }
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  {
    if (IsDebounced(&wkupLastTick))
    {
      HAL_GPIO_TogglePin(LED0_GPIO_PORT, LED0_GPIO_PIN);
    }
  }
  else if (GPIO_Pin == KEY0_GPIO_PIN)
  {
    if (IsDebounced(&key0LastTick))
    {
